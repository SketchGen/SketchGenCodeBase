...
public class AlloyProgram {
  ...
  public AlloyProgram() {
    this.program = new ArrayList<List<AlloyFragment>>();
  ...
  public void addFragment(String content, Integer lineNumber) {
    ...
    if (lineNumber >= program.size()) { // Add new line
      program.add(new ArrayList<AlloyFragment>());
    }
    // Add new fragment
  ...
  public void addHole(String content, Integer lineNumber, int begin, int end)
      throws UnsupportedHoleException {
  ...
  private Hole createHole(String sketchType, Integer lineNumber, Integer begin, Integer end)
      throws UnsupportedHoleException {
    switch (sketchType) {
      case SQ:
        return new SQ(sketchType, lineNumber, begin, end);
      case Q:
        return new Q(sketchType, lineNumber, begin, end);
      case LO:
        return new LO(sketchType, lineNumber, begin, end);
      case CO:
        return new CO(sketchType, lineNumber, begin, end);
      case UO:
        return new UO(sketchType, lineNumber, begin, end);
      case UOF:
        return new UOF(sketchType, lineNumber, begin, end);
      case UOE:
        return new UOE(sketchType, lineNumber, begin, end);
      case BO:
        return new BO(sketchType, lineNumber, begin, end);
      case E:
        return new E(sketchType, lineNumber, begin, end);
      default:
        throw new UnsupportedHoleException(sketchType + " is not supported.");
    }
  }
